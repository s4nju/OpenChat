...
export type ConnectorType = 'gmail' | 'googlecalendar' | 'notion' | 'googledrive';
...
export const getAuthConfigId = (connectorType: ConnectorType): string => {
  switch (connectorType) {
    ...
    case 'googlecalendar':
      return process.env.NEXT_PUBLIC_CALENDAR_AUTH_CONFIG_ID || 'googlecalendar_oauth';
    case 'notion':
      return process.env.NEXT_PUBLIC_NOTION_AUTH_CONFIG_ID || 'notion_oauth';
    case 'googledrive':
      return process.env.NEXT_PUBLIC_GOOGLE_DRIVE_AUTH_CONFIG_ID || 'googledrive_oauth';
    default:
